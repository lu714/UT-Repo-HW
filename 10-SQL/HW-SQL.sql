...
SELECT t3.first_name, t3.last_name, t3.email
FROM city t1 
JOIN address t2 ON t1.city_id = t2.city_id
JOIN customer t3 ON t2.address_id = t3.address_id
WHERE t1.country_id = (SELECT country_id FROM country WHERE country = "Canada");
...
SELECT f.title, COUNT(r.customer_id) AS 'rental_freq'
FROM film f
